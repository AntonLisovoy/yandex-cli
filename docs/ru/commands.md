...
| Флаг | Что делает |
|---|---|
| `--json` | Печатает ровно один документ JSON вместо таблицы. |
| `--out FILE` | Пишет результат в файл `FILE` и печатает только краткую сводку; включает `--json`. |
| `--select PATH,...` | Оставляет в результате только перечисленные пути через точку; включает `--json`. |
| `--pretty` | Форматирует JSON с отступами для чтения; по умолчанию он компактный. |
| `--profile NAME` | Использует сохранённый профиль конфигурации для этой команды. |
| `--read-only` / `--wiki-read-only` | Запрещает команды записи в Tracker / Wiki. |
| `--limit-queues Q1,Q2` | Ограничивает, каких очередей вообще можно коснуться. |
| `--timeout SECONDS` | HTTP-таймаут для этой команды. |

`--profile` и флаги режима «только чтение»/ограничения очередей разобраны в
[настройке](configuration.md); `--out`, `--select` и `--pretty` разобраны
ниже, в разделе [JSON для скриптов](#json-для-скриптов).
...
Обычный инструмент, чтобы выбрать нужное из результата, — `jq`. Сначала
включите `set -o pipefail`: без него конвейер съедает код возврата CLI, и
конверт `{"error": ...}` неудачной команды превращается в `null` от `jq`
вместо того, чтобы скрипт заметил, что что-то пошло не так.
...
### Отправить результат в файл вместо терминала

Содержимое страницы wiki или описание задачи может оказаться гораздо больше,
чем нужно на экране — а для агента, который управляет CLI, ещё и больше, чем
нужно в его контексте. `--out FILE` пишет результат в файл `FILE` и печатает
вместо него только краткую сводку:
...
Длинное текстовое поле — содержимое wiki-страницы, описание задачи —
записывается в файл как **обычный текст**, а не как JSON-строка с
экранированными переводами строк, так что `grep -n "..." kb.md` и
`sed -n '120,180p' kb.md` работают с ним напрямую. Структурированный результат
(список, очередь, доска) записывается как JSON, и тогда сводка сообщает
`items`, а не `lines`.

`--out` включает `--json`, молча перезаписывает существующий файл, ничего не
пишет, если команда завершилась ошибкой, и отклоняется на четырёх командах, у
которых уже есть собственный файл через `--output` или `-o`:
`issue attachment-download`, `wiki attachment download`,
`wiki attachment download-url` и `export issues`. Внутри REPL он не действует.

### Сузить результат на своей стороне флагом `--select`

`--select PATH,...` оставляет только перечисленные пути через точку, без
языка запросов. Результат сохраняет вложенность, так что написанные под него
пути `jq` продолжают работать:
...
Путь пишется так, как поле называется в выводе, — `updatedAt`, а не
`updated_at`. Путь, который упирается в список, применяется к каждому его
элементу: так `results.title` выше превращает каждый результат поиска в одно
только его заглавие. Путь, которому ничего не соответствует, даёт `null` —
отсутствующее поле и настоящий `null` неотличимы друг от друга после
сериализации результата, поэтому стоит перепроверить путь, прежде чем решить,
что поле действительно пустое. Применить `--select` к результату, который
является одним скаляром, а не объектом или списком, — ошибка. `--select`
тоже включает `--json` и не действует внутри REPL.

### Читать JSON глазами флагом `--pretty`

По умолчанию `--json` печатает компактный JSON — в расчёте на то, что скрипту
пробелы безразличны. Добавьте `--pretty`, когда вывод будет читать человек: он
форматирует тот же документ отступами, не меняя его содержимого, и сочетается
с `--out` и `--select` точно так же, как `--json`.
